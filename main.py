...
class Categorization_Service(Resource):
    def post(self, webStoreId):
        ...
        data_Galmart = pd.read_csv('Galmart_Astana_raw')
        data_Galmart = categorize(data_Galmart)
        # posting the categorized Galmart products via API:
        code = requests.post('http://13.59.5.143:8082/webcatalogitems/excel', 'Galmart_Astana.xlsx', webStoreId)
        return code

api.add_resource(Scrapping_Service, '/scrap_csv/<string:storeName>/<string:city>/<string:url>')
api.add_resource(Categorization_Service, '/categorize/<long:webStoreId>')
